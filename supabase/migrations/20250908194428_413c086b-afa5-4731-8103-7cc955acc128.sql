...
CREATE POLICY "Admins can view all completion trends" ON completion_trends FOR SELECT USING (is_admin(auth.uid()));
CREATE POLICY "Admins can insert completion trends" ON completion_trends FOR INSERT WITH CHECK (is_admin(auth.uid()));
CREATE POLICY "Admins can update completion trends" ON completion_trends FOR UPDATE USING (is_admin(auth.uid()));
CREATE POLICY "Admins can delete completion trends" ON completion_trends FOR DELETE USING (is_admin(auth.uid()));