...
-- Update RLS policy to allow DMs to insert approvals (recreate the policy)
DROP POLICY IF EXISTS "Executives can insert their own approvals" ON critical_feedback_approvals;
DROP POLICY IF EXISTS "Executives and DMs can insert their own approvals" ON critical_feedback_approvals;
